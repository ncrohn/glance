...
export function mountBlockExpanders(view: HTMLElement): void {
  const blocks = view.querySelectorAll<HTMLElement>(
    "pre:not(.mermaid-block), .table-scroll",
  );
  for (const block of blocks) {
    ...
    // the wrapper (not the scroll container) so it stays put when the code/table
    // is scrolled horizontally. The wrapper is also the element that animates
    // its width when expanding.
